...
export default function AdminEvaluationDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const evaluators = users.filter(u => u.role === 'EVALUATOR' || u.role === 'ADMIN'); // ADM, EVAL
    const evaluatees = users.filter(u => u.role === 'EVALUATEE' || u.role === 'ADMIN' || u.role === 'EVALUATOR'); // Everyone can be evaluated if needed
                            ...
                            <table className="w-full text-left whitespace-nowrap border-collapse">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-sm">
                                        ...
                                        <th className="py-3 px-4 font-semibold text-center">หัวข้อที่ประเมิน</th>
                                        <th className="py-3 px-4 font-semibold text-center">คะแนนดิบ</th>
                                        <th className="py-3 px-4 font-semibold text-center">% จากน้ำหนัก</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {evaluation?.assignments?.map((a: any) => {
                                        const hasResults = a.results && a.results.length > 0;
                                        if (!hasResults) return null; // Skip non-evaluated ones in this summary? Or show them. Let's show all for clarity.

                                        // Calculate total score percentage. Need logic! 
                                        // Assuming weight of each indicator is max points? 
                                        // Results usually store 'score' out of say 5. 
                                        return (
                                            <tr key={a.id} className="border-b border-slate-100 hover:bg-slate-50 text-sm">
                                                <td className="py-3 px-4">{a.evaluatee?.name}</td>
                                                <td className="py-3 px-4 text-slate-500">{a.evaluator?.name}</td>
                                                <td className="py-3 px-4 text-center">{hasResults ? a.results.length : 0} / ตัวชี้วัดทั้งหมด</td>
                                                <td className="py-3 px-4 text-center">{hasResults ? 'มีผลประเมิน' : '-'}</td>
                                                <td className="py-3 px-4 text-center font-bold text-primary-600">
                                                    {/* Further score calculation requires more detailed result schema data */}
                                                </td>
                                            </tr>
                                        );
                                    })}
                                    {(!evaluation?.assignments?.find((a: any) => a.results && a.results.length > 0)) && (
                                        <tr>
                                            <td colSpan={5} className="py-8 text-center text-slate-500">
                                                ยังไม่มีผลการประเมินในระบบ
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
